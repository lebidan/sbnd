...
class SBNDLitModule(LightningModule):
    ...
    def on_train_epoch_start(self) -> None:
        ...
        self._grad_norm_acc = []
        self._adam_step_max_acc = []


    def on_train_epoch_end(self):
        # log cumulated layer norms
        layer_norms = {name: p.detach().norm().item() for name, p in self.named_parameters()}
        metrics = {"cum_weight_norm": sum(layer_norms.values())}        

        # log gradient stats
        if self._grad_norm_acc:
            metrics["grad_norm_mean"] = sum(self._grad_norm_acc) / len(self._grad_norm_acc)
            ...
            if not torch.isfinite(all_params).all():
                ...
                for name, p in self.named_parameters():
                    if not torch.isfinite(p).all():
                        raise ValueError(f"[step {self.global_step}] NaN/Inf found in {name}")        
