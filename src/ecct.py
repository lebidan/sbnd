...
class Encoder(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        for idx, layer in enumerate(self.layers, start=1):
            x = layer(x, mask)
            # MODIF RLB:
            # The following two lines appear to be the main cause of some
            # spikes in the training curves, and this middle norm is not
            # common practice in transformer architectures
                ...
                for jj in idx:
                    for kk in idx:
                        if jj != kk:
                            ...
                            mask[jj, code.n + ii] += 1
            src_mask = (
                ~(mask > 0).unsqueeze(0).unsqueeze(0)
            )  # mask=True for elements that do NOT participate to attention
            return (
                ~src_mask if invert else src_mask
            )  # invert the mask for Pytorch SDPA (opposite convention)
