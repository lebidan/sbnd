...
`ebno_dB_train` can also be a list, in which case each training batch is a deterministic mixture of samples drawn at the listed SNRs. The optional `batch_mix` argument controls the proportion of samples drawn at each SNR (defaults to uniform; values are normalized to sum to 1). Mixing happens *within* each batch — every gradient step sees a representative spread of easy and hard samples — rather than across batches, which would bias each step toward whichever SNR happened to be drawn for it.
...
> **Pick the mix deliberately — the default uniform is rarely the right choice.** Decoder models often deal well with high-SNR samples: most of them are corrected with little gradient signal. Conversely, low-SNR samples can be too hard to learn. Tune empirically to find the right match for your code and operating conditions. The ICASSP 2023 paper by [Wiesmayr et al](https://arxiv.org/abs/2210.14103) can be a good starting point. See also [per-group loss reweighting](#per-group-loss-reweighting) for a complementary knob.

Per-batch sample counts are computed deterministically from `batch_mix` and `train_bs` (proportional rounding, with a one-sample floor for non-zero entries so every requested SNR is represented). Every batch hits the requested proportions exactly, eliminating per-batch composition variance.
...
**DDP.** Multi-file training is DDP-safe: per-file shuffles are derived deterministically from `(base_seed, epoch)` so all ranks compute identical index lists, and Lightning's auto-installed `DistributedSampler` over batch indices distributes disjoint batches across ranks. The only caveat is that `persistent_workers=True` is not supported in this mode (per-epoch reshuffles wouldn't propagate to persistent workers); the DataModule force-disables it with a warning.
...
data:
  ebno_dB_train: [2.0, 3.0, 4.0]
  batch_mix:     [0.6, 0.3, 0.1]     # per-batch composition (proportions)
