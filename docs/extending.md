...
* **Adding a code automorphism family** — implementing a new permutation class in [`src/transforms.py`](../src/transforms.py), used both for training-time data augmentation and for [test-time augmentation](evaluation.md#test-time-augmentation). The contract is small: subclass `BasePerms`, call `super().__init__(code)`, populate `self.perms` (shape `(n_perms, code.n)`) and `self.n_perms`, and provide a `__call__(y, e) → (yp, ep)` method for the training-augmentation path. Both `sample_perms(bs) → (perms, perms_inv, synd_maps)` and the lazy build of `self.synd_maps` (only triggered on the first `sample_perms` call, so training-only callers never pay for it) are inherited from `BasePerms`.
* **Adding a TTS variant** — implementing a new decoding strategy in [`src/tts.py`](../src/tts.py). The protocol is `decode(model, code, ym, syndromes) → preds`, plus the `name`, `suffix`, and `validate(model, code)` attributes.
